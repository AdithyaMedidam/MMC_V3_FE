...
export default function UpdateAffiliationForm({ onClose, initialData }) {
    ...
    // Prepare initial programs array
    const getInitialPrograms = () => {
        if (!initialData?.programs) return [{ value: "" }];
        const programs = Array.isArray(initialData.programs) 
            ? initialData.programs 
            : [initialData.programs];
        return programs.map(p => ({ value: p || "" }));
    };
    ...
    const { 
        ...
        defaultValues: {
            ...
            renewalDate: parseDate(initialData?.renewal),
    ...
    useEffect(() => {
        if (initialData) {
            const initialPrograms = !initialData?.programs 
                ? [{ value: "" }]
                : (Array.isArray(initialData.programs) 
                    ? initialData.programs 
                    : [initialData.programs]).map(p => ({ value: p || "" }));
